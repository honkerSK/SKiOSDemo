...
const char *kSKPropertyListKey = "SKPropertyListKey";
    ...
    [dict enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        if ([propertyList containsObject:key]) {
            ...
            if ([obj isKindOfClass:NSClassFromString(@"__NSCFDictionary")]) { //  是字典对象,并且属性名对应类型是自定义类型
                ...
                //判断对应类有没有实现字典数组转模型 的字典协议方法
                if (![self respondsToSelector:@selector(dictWithModelClass)]) return ;
                
                // value:user字典 -> User模型
                // 获取模型(user)类对象
                NSString *ivarType = [ShopItem dictWithModelClass][key];
                Class modalClass = NSClassFromString(ivarType);
                
                // 字典转模型
                if (modalClass) {
                    // 字典转模型 user
                    obj = [modalClass sk_modelWithDict:obj];
                }
            ...
            if ([obj isKindOfClass:[NSArray class]]) {
                ...
                if (![self respondsToSelector:@selector(arrayContainModelClass)])
                    ...
                    for (NSDictionary *dict in obj) {
                        // 字典转模型
                        id model =  [classModel sk_modelWithDict:dict];
...
+ (NSArray *)sk_objcProperties {
    ...
    objc_setAssociatedObject(self, kSKPropertyListKey, mtArray.copy, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    /* 释放 */
    free(propertyList);
    return mtArray.copy;
